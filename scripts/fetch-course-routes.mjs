...
function verdict(job, r) {
  ...
  /**
   * **희박하면서 곧은 선은 길이 아니라 지름길이다.** (2026-09-08 추가)
   *
   * 뚝섬이 이 조합으로 두 번 통과했다 — 2.24km, 단순화 점 5개, 비율 1.01배.
   * 화면을 보니 **한강을 가로지르는 직선**이었다. hint 를 공식 좌표로 바꿔도
   * 같은 값이 나왔다(원인은 hint 가 아니라 그 bbox 의 길 연결 구조였다).
   *
   * 근거: 강변 산책로는 **강이 굽으니 같이 굽고**, OSM 노드도 촘촘하다.
   *   · 잠실  16점 / 3.24km = 4.9점/km, 1.12배
   *   · 반포  18점 / 1.91km = 9.4점/km, 1.15배
   *   · 뚝섬   5점 / 2.24km = 2.2점/km, 1.01배  <- 이것만 다르다
   * 두 조건을 **동시에** 만족할 때만 떨어뜨린다. 하나만으로는 정상 구간도
   * 걸릴 수 있다(짧고 곧은 구간은 실제로 있다).
   *
   * ⚠️ 이건 **어림짐작이다.** 수치로 "선이 물 위에 있다"를 확인할 수는 없다.
   * 최종 판정은 `npm run shot` 으로 화면을 보는 것이다 — 이 검사는 그 전에
   * 명백한 것만 걸러낸다.
   */
  const perKm = r.coords.length / r.km;
  if (perKm < 3 && ratio < 1.05)
    problems.push(
      `점 ${perKm.toFixed(1)}개/km 에 비율 ${ratio.toFixed(2)}배 — 희박하고 곧다. 길이 아니라 지름길로 보인다`
    );
  /**
